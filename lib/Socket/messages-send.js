            ...
            if (isGroupOrStatus && !isRetryResend) {
                ...
                if (senderKeyRecipients.length) {
                    ...
                    try {
                        ...
                        const isRecoverable = ['not-acceptable', 'Timed Out', 'No sessions'].includes(sessionErr.message);
                        if (isRecoverable && senderKeySessionTargets.length > 1) {
                            logger.warn({ count: senderKeySessionTargets.length, err: sessionErr.message }, 'batch assertSessions failed — retrying per-JID to skip unreachable devices');
                            reachableRecipients = [];
                            for (const candidateJid of senderKeySessionTargets) {
                                try {
                                    ...
                                    logger.warn({ jid: candidateJid, err: e.message }, 'skipping unreachable participant device');
                                }
                            }
                        }
                        else {
